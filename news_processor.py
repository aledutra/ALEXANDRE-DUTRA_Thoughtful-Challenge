...
class News:
    ...
    def __init__(self, web_element: WebElement, search_phrase: str):
        ...
        self.count_phrase: Optional[int] = None
        self.contain_money: Optional[bool] = None

    def parse_title(self) -> None:
        """Parse and set the title of the news item."""
        try:
            self.title = self.web_element.find_element(
                By.CSS_SELECTOR, '[data-testid="Heading"]'
            ).text
    ...
    def parse_date(self) -> None:
        ...
        try:
            date_elem = self.web_element.find_element(
                By.CSS_SELECTOR, 'time[data-testid="Text"]'
            )
    ...
    def parse_image_link(self) -> None:
        ...
        try:
            ...
            if noscript_elements:
                ...
                image_link_elem = self.web_element.find_element(
                    By.CSS_SELECTOR, "[src]"
                )
                self.image_link = image_link_elem.get_attribute("src")
            logger.info(
                f"Parsed image link: {self.image_link[:20]}......{self.image_link[-20:]}"
            )
        except NoSuchElementException:
    ...
    def count_search_phrases(self) -> None:
        ...
        if self.title:
            self.count_phrase = len(
                re.findall(re.escape(self.search_phrase), self.title, re.IGNORECASE)
            )
    ...
    def contains_money(self) -> None:
        ...
        if self.title:
            ...
            self.contain_money = bool(
                re.search(money_pattern, self.title, re.IGNORECASE)
            )
...
class NewsProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized NewsProcessor with search phrase: '{search_phrase}', section: '{section}', date range: {date_range} months"
        )
    ...
    def define_url(self, offset: int = 0) -> str:
        ...
        url = (
            f"{base_url}{encoded_query}&offset={offset}&section={category}&sort=newest"
        )
    ...
    def process_news(self, news_elements: List[WebElement]) -> bool:
        ...
        for element in news_elements:
            ...
            if self.is_within_date_range(news.date):
                ...
                logger.info(
                    f"Added news item to report. Current report size: {len(self.report)}"
                )
            else:
    ...
    def iterate_over_pages(self) -> None:
            ...
            try:
                ...
                WebDriverWait(self.driver, 10).until(
                    EC.presence_of_element_located(
                        (By.CLASS_NAME, "search-results__list__2SxSK")
                    )
                )
                ...
                if offset == 0:
                    ...
                    count_news_elem = WebDriverWait(self.driver, 10).until(
                        EC.presence_of_element_located(
                            (By.CLASS_NAME, "search-results__subtitle__3k4lv")
                        )
                    )
                    count_news_elem = count_news_elem.find_element(
                        By.CSS_SELECTOR, '[data-testid="Text"]'
                    )
                ...
                news_list = self.driver.find_element(
                    By.CLASS_NAME, "search-results__list__2SxSK"
                )
                news_items = news_list.find_elements(
                    By.CLASS_NAME, "search-results__item__2oqiX"
                )
                ...
                logger.info(
                    f"Processed {len(self.report)} news items within the date range"
                )
                logger.info(f"Total news processed so far: {total_news_processed}")

                if not continue_processing:
                    logger.info(
                        "Reached news outside of the specified date range. Stopping."
                    )
                    break

                if total_news_processed >= total_news:
                    logger.info(
                        f"Reached or exceeded the total number of news items ({total_news}). Stopping."
                    )
                    break
        ...
        logger.info(
            f"Finished iterating over pages. Total news items in report: {len(self.report)}"
        )

    def download_image(self, news: News, download_path: str) -> str:
        """
        ...
        try:
            ...
            with open((download_path), "wb") as file:
                file.write(
                    self.driver.find_element(By.TAG_NAME, "img").screenshot_as_png
                )
                ...
                for _ in range(10):
                    ...
                    logger.warning(
                        f"Timeout while waiting for image download: {news.image_name}"
                    )
    ...
    def save_to_excel(self) -> None:
        """Save processed news data to an Excel file and download the news pictures."""
        try:
    ...
    def process(self) -> List[News]:
        ...
        try:
            ...
            self.save_to_excel()
            logger.info(
                f"Finished processing. Total news items collected: {len(self.report)}"
            )
            return "Successful"
        except Exception as e:
